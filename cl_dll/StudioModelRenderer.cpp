...
#ifdef CLIENT_DLL
extern bool g_bCrossbowScopeVisible;
#endif
...
#ifdef CLIENT_DLL
static void DrawCrossbowScopeOverlay()
{
	if (!g_bCrossbowScopeVisible)
		return;

	static const char* kScopeSpriteNames[] = {
		"crossbow_scope",
		"sniper_scope",
		"scope"
	};

	HSPRITE hScopeSprite = NULL;
	const wrect_t* pScopeRect = NULL;

	for (size_t i = 0; i < sizeof(kScopeSpriteNames) / sizeof(kScopeSpriteNames[0]); ++i)
	{
		int spriteIndex = gHUD.GetSpriteIndex(kScopeSpriteNames[i]);
		if (spriteIndex < 0)
			continue;

		hScopeSprite = gHUD.GetSprite(spriteIndex);
		pScopeRect = &gHUD.GetSpriteRect(spriteIndex);
		break;
	}

	if (!hScopeSprite || !pScopeRect)
		return;

	SPR_Set(hScopeSprite, 255, 255, 255);
	SPR_DrawHoles(0, 0, 0, pScopeRect);
}
#endif
...
#ifdef CLIENT_DLL
	if (g_bCrossbowScopeVisible && m_pCurrentEntity == gEngfuncs.GetViewModel() && m_pCurrentEntity->model && strcmp(m_pCurrentEntity->model->name, "models/v_crossbow.mdl") == 0)
	{
		DrawCrossbowScopeOverlay();
		return 0;
	}
#endif
